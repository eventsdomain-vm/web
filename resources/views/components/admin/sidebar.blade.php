...
<nav class="flex-1 overflow-y-auto py-6 px-4 no-scrollbar">
    <div class="flex flex-col gap-6">
        @foreach($adminNavItems as $group)
            <div>
                ...
                <ul class="flex flex-col gap-0.5">
                    @foreach($group['items'] as $item)
                        @php $hasSubmenu = isset($item['submenu']) && isSubmenuActive($item['submenu']); @endphp
                        <li x-data="{ open: @js($hasSubmenu) }">
                            @if(isset($item['submenu']))
                                <button
                                    @click="open = !open"
                                    class="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition group {{ isSubmenuActive($item['submenu']) ? 'bg-[#F26C4F]/10 text-[#E35336]' : 'text-gray-600 hover:bg-gray-100 hover:text-gray-900' }}"
                                >
                                    <svg class="w-5 h-5 shrink-0 {{ isSubmenuActive($item['submenu']) ? 'text-[#E35336]' : 'text-gray-400 group-hover:text-gray-600' }}" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="{{ $item['icon'] }}"/>
                                    </svg>
                                    <span class="flex-1 text-left">{{ $item['label'] }}</span>
                                    <svg class="w-4 h-4 transition-transform duration-200 {{ isSubmenuActive($item['submenu']) ? 'text-[#E35336]' : 'text-gray-400' }}" :class="{ 'rotate-180': open }" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7"/>
                                    </svg>
                                </button>
                                <ul x-show="open" x-collapse.duration.200ms class="mt-0.5 space-y-0.5 ml-4">
                                    @foreach($item['submenu'] as $subItem)
                                        @php $subActive = isSubItemActive($subItem); @endphp
                                        <li>
                                                <a
                                                    href="{{ route($subItem['route']) }}{{ isset($subItem['query']) ? '?' . $subItem['query'] : '' }}"
                                                class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition {{ $subActive ? 'bg-[#F26C4F] text-white shadow-sm' : 'text-gray-500 hover:bg-gray-100 hover:text-gray-900' }}"
                                            >
                                                <span class="w-1.5 h-1.5 rounded-full {{ $subActive ? 'bg-white' : 'bg-gray-300' }}"></span>
                                                {{ $subItem['label'] }}
                                            </a>
                                        </li>
                                    @endforeach
                            ...
                            @else
                                ...
                                <a
                                    ...
                                    class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition {{ $active ? 'bg-[#F26C4F] text-white shadow-sm' : 'text-gray-600 hover:bg-gray-100 hover:text-gray-900' }}"
                                >
