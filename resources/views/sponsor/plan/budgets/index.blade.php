<x-app-layout>
    ...
    <div class="py-6 max-w-4xl mx-auto">
        ...
        @if($budgetAllocations->isNotEmpty())
            <div class="space-y-5">
                @foreach($budgetAllocations as $allocation)
                        ...
                        <div class="p-6">
                            <div class="flex flex-wrap items-start justify-between gap-4 mb-5">
                                <div>
                                    <div class="flex items-center gap-3">
                                        <h4 class="font-semibold text-lg text-gray-900">{{ $allocation->fiscal_year }} Fiscal Year</h4>
                                        <span class="px-2.5 py-0.5 rounded-full text-xs font-medium {{ $badge }}">{{ ucfirst($allocation->status) }}</span>
                                    </div>
                                    <p class="text-sm text-gray-500 mt-1">
                                        {{ $allocation->valid_from?->format('M d, Y') ?? '—' }}
                                        @if($allocation->valid_to) &rarr; {{ $allocation->valid_to->format('M d, Y') }} @endif
                                    </p>
                                </div>
                                <div class="text-right">
                                    <p class="text-xs text-gray-500 uppercase tracking-wide">Total Budget</p>
                                    <p class="text-2xl font-bold text-gray-900">₹{{ number_format($allocation->total_budget, 2) }}</p>
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-2 gap-4 mt-5">
                                <div class="p-3.5 bg-gray-50 rounded-xl">
                                    ...
                                    <p class="text-sm font-semibold text-gray-900">₹{{ number_format($allocation->allocated_so_far, 2) }}</p>
                                </div>
                                <div class="p-3.5 bg-gray-50 rounded-xl">
                                    <p class="text-xs text-gray-500">Remaining</p>
                                    <p class="text-sm font-semibold text-gray-900">₹{{ number_format($remaining, 2) }}</p>
                                </div>
                            </div>

                            @if($allocation->category_allocations)
                                <div class="mt-5">
                                    <p class="text-xs font-medium text-gray-500 uppercase tracking-wide mb-2">Category Split</p>
                                    <div class="flex flex-wrap gap-2">
                                        @foreach($allocation->category_allocations as $category => $amount)
                                            <span class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-terracotta-50 text-terracotta-700 text-sm">
                                                <span class="font-medium">{{ $category }}</span>
                                                <span class="text-terracotta-500">₹{{ number_format($amount, 2) }}</span>
                                            </span>
                                        @endforeach
                                    </div>
                                </div>
                            ...
                            <div class="flex items-center justify-end gap-3 mt-5 pt-4 border-t border-gray-100">
                                <button x-data @click="$dispatch('open-modal', 'edit-budget-{{ $allocation->id }}')" class="text-terracotta-500 hover:underline text-sm font-medium">Edit</button>
                                <form action="{{ route('sponsor.plan.budgets.delete', $allocation) }}" method="POST" x-data onsubmit="return confirm('Delete this budget allocation?')">
                                    @csrf
                                    @method('DELETE')
                    ...
                    {{-- Edit Modal --}}
                    <x-modal name="edit-budget-{{ $allocation->id }}">
                        <div class="p-6">
                            <h3 class="text-lg font-semibold text-gray-900 mb-1">Edit Budget Allocation</h3>
                            <p class="text-sm text-gray-500 mb-5">{{ $allocation->fiscal_year }} Fiscal Year</p>
                            <form x-data="budgetForm()" x-ref="form" action="{{ route('sponsor.plan.budgets.update', $allocation) }}" method="POST"
                                ...
                                <div class="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <input type="text" name="fiscal_year" value="{{ $allocation->fiscal_year }}" pattern="[0-9]{4}" class="w-full rounded-lg border-gray-200 text-sm" required>
                                    </div>
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700 mb-1.5">Total Budget (₹)</label>
                                        <input type="number" name="total_budget" value="{{ $allocation->total_budget }}" step="0.01" min="0" class="w-full rounded-lg border-gray-200 text-sm" required>
                                    </div>
                                </div>
                                ...
                                <div>
                                    ...
                                    <select name="status" class="w-full rounded-lg border-gray-200 text-sm">
                                        <option value="draft" {{ $allocation->status === 'draft' ? 'selected' : '' }}>Draft</option>
                                        <option value="approved" {{ $allocation->status === 'approved' ? 'selected' : '' }}>Approved</option>
                                        <option value="active" {{ $allocation->status === 'active' ? 'selected' : '' }}>Active</option>
                                        <option value="closed" {{ $allocation->status === 'closed' ? 'selected' : '' }}>Closed</option>
                                    </select>
                                </div>
                                ...
                                <div class="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <input type="date" name="valid_from" value="{{ $allocation->valid_from?->format('Y-m-d') }}" class="w-full rounded-lg border-gray-200 text-sm">
                                    </div>
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700 mb-1.5">Valid To</label>
                                        <input type="date" name="valid_to" value="{{ $allocation->valid_to?->format('Y-m-d') }}" class="w-full rounded-lg border-gray-200 text-sm">
                                    </div>
                                </div>

                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-2">Category Allocations</label>
                                    <div x-data='{ rows: @json($catRows) }' class="space-y-3">
    ...
    {{-- Create Budget Modal --}}
    <x-modal name="create-budget-modal">
        <div class="p-6">
            ...
            <form x-data="budgetForm()" x-ref="form" action="{{ route('sponsor.plan.budgets.store') }}" method="POST"
                  @submit.prevent="submit($refs.form)" class="space-y-5">
                @csrf
