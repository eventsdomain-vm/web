<x-guest-layout>
    <x-slot name="title">{{ $event->title }} - EventsDomain</x-slot>
    <x-slot name="meta_description">{{ $event->tagline ?? Str::limit(strip_tags($event->description), 160) }}</x-slot>
            ...
            <div class="container-page">
                <div class="max-w-4xl">
                    <div class="flex flex-wrap items-center gap-2 mb-4">
                        <span class="bg-white/15 text-white text-xs px-3 py-1 rounded-full font-medium backdrop-blur-sm">{{ $event->category->name ?? 'General' }}</span>
                        ...
                        @if($event->sponsorship_type)
                            <span class="bg-emerald-500/20 text-emerald-300 text-xs px-3 py-1 rounded-full font-medium border border-emerald-500/30">{{ ucfirst($event->sponsorship_type) }}</span>
                        @endif
                    ...
                    <h1 class="text-3xl md:text-5xl font-bold text-white mb-3 leading-tight">{{ $event->title }}</h1>

                    @if($event->tagline)
                        <p class="text-base md:text-lg text-white/70 mb-5 max-w-2xl">{{ $event->tagline }}</p>
                    @endif
                    ...
                    <div class="flex flex-wrap items-center gap-x-5 gap-y-2 text-sm text-white/60">
                        <span class="flex items-center gap-1.5">
                            ...
                            {{ $event->start_date->format('M d, Y') }} @if($event->end_date && $event->end_date != $event->start_date) - {{ $event->end_date->format('M d, Y') }}@endif
                        </span>
                        <span class="flex items-center gap-1.5">
                        ...
                        <span class="flex items-center gap-1.5">
                            ...
                            {{ number_format($event->expected_audience) }} Expected
                        </span>
                        @if($event->budget_min || $event->budget_max)
                            <span class="flex items-center gap-1.5 text-terracotta-300 font-semibold">
                                ...
                                @if($event->budget_min && $event->budget_max)
                                    ₹{{ number_format($event->budget_min) }} - ₹{{ number_format($event->budget_max) }}
                                @elseif($event->budget_min)
                                    From ₹{{ number_format($event->budget_min) }}
                                @else
                                    Up to ₹{{ number_format($event->budget_max) }}
                                @endif
                            </span>
                        @endif
                    </div>
    ...
    ═══════════════════════════════════════════ --}}
    @if($galleryImages->isNotEmpty() && $galleryImages->count() > 1)
            ...
            <div class="container-page py-4">
                <div class="flex gap-2 overflow-x-auto pb-2 scrollbar-thin">
                    @foreach($galleryImages as $i => $imgUrl)
                        <button
                            @click="$dispatch('gallery-change', { index: {{ $i }} })"
                            class="flex-shrink-0 w-28 h-20 rounded-lg overflow-hidden bg-gray-100 hover:ring-2 transition focus:outline-none"
                            :class="active === {{ $i }} ? 'ring-2 ring-terracotta-500' : 'ring-0 hover:ring-1 hover:ring-gray-300'"
                        >
                            <img src="{{ $imgUrl }}" alt="{{ $event->title }}" class="w-full h-full object-cover" loading="lazy" onerror="this.outerHTML='<div class=\'w-full h-full bg-gray-200\'></div>'">
                        </button>
                    @endforeach
                </div>
            </div>
    ...
    <nav class="bg-white border-b border-gray-100">
        <div class="container-page py-3">
            <ol class="flex items-center space-x-2 text-sm text-gray-500">
                ...
                <li><a href="{{ route('events.index') }}" class="hover:text-terracotta-600 transition">Explore Events</a></li>
                <li><svg class="w-4 h-4 text-gray-300" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg></li>
                <li class="text-gray-900 font-medium truncate max-w-[200px] md:max-w-xs">{{ $event->title }}</li>
            </ol>
        </div>
    </nav>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div class="card p-6 md:p-8">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
                            ...
                            <div class="bg-sky-50 rounded-xl p-4 text-center">
                                <div class="text-2xl font-bold text-sky-600">{{ $event->expected_audience ? number_format(ceil($event->expected_audience * 0.4)) : 'N/A' }}</div>
                        ...
                        @forelse($event->packages as $package)
                            <div class="border border-gray-200 rounded-2xl p-6 mb-4 last:mb-0 hover:border-terracotta-200 transition hover:shadow-sm">
                                ...
                                <div class="flex flex-wrap items-center gap-3 text-sm text-gray-500 mb-4">
                                    ...
                                    @endif
                                    @if($package->slots_available > $package->slots_filled)
                                        <span class="w-1 h-1 bg-gray-300 rounded-full"></span>
                                        <span class="text-emerald-600 font-medium">{{ $package->slots_available - $package->slots_filled }} slots left</span>
                                    @else
                                        <span class="w-1 h-1 bg-gray-300 rounded-full"></span>
                                    ...
                                    <div class="mt-5">
                                        @auth
                                            @if(auth()->user()->hasRole('sponsor'))
                                                <a href="{{ route('sponsor.events.show', $event) }}" class="btn-primary text-sm">Sponsor This Package</a>
                                            @else
                                                <a href="{{ route('login') }}" class="btn-primary text-sm">Login to Sponsor</a>
                                            @endif
                                        @else
                                            <a href="{{ route('register') }}" class="btn-primary text-sm">Create Account to Sponsor</a>
                                        @endauth
                                    </div>
                                @endif
                            </div>
                ...
                <div class="space-y-6">
                    <div class="card p-6 sticky top-24">
                        ...
                        @auth
                            @if(auth()->user()->hasRole('sponsor'))
                                <a href="{{ route('sponsor.events.show', $event) }}" class="btn-primary w-full text-center block">
                            ...
                            @else
                                <a href="{{ route('login') }}" class="btn-primary w-full text-center block">Login to Sponsor</a>
                            @endif
                        @else
                            <a href="{{ route('register') }}" class="btn-primary w-full text-center block">Create Account to Sponsor</a>
                        @endauth
                    </div>
                    ...
                    <div class="card p-6">
                        ...
                        <div class="space-y-3">
                            ...
                            <div class="flex items-center justify-between py-2 border-b border-gray-50">
                                ...
                                <span class="text-sm font-semibold text-gray-900">{{ $galleryImages->count() }} images</span>
                            </div>
                            @endif
                    ...
                    <div class="card p-6">
                        ...
                        <div class="flex gap-3">
                            <a href="https://twitter.com/intent/tweet?url={{ urlencode(request()->fullUrl()) }}&text={{ urlencode($event->title) }}" target="_blank" class="w-10 h-10 bg-blue-50 rounded-xl flex items-center justify-center hover:bg-blue-100 transition" title="Share on X">
                            ...
                            <a href="https://www.facebook.com/sharer/sharer.php?u={{ urlencode(request()->fullUrl()) }}" target="_blank" class="w-10 h-10 bg-blue-50 rounded-xl flex items-center justify-center hover:bg-blue-100 transition" title="Share on Facebook">
                            ...
                            <a href="https://wa.me/?text={{ urlencode($event->title . ' - ' . request()->fullUrl()) }}" target="_blank" class="w-10 h-10 bg-green-50 rounded-xl flex items-center justify-center hover:bg-green-100 transition" title="Share on WhatsApp">
