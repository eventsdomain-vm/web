                    ...
                    <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6 transition hover:shadow-md">
                        ...
                        @if($account)
                            ...
                            <div class="flex gap-2">
                                <a href="{{ route($rp . '.social.connect', $key) }}" class="flex-1 text-center px-4 py-2.5 text-sm font-medium text-gray-700 bg-white border border-gray-200 rounded-lg hover:bg-gray-50 transition">
                                    Reconnect
                                </a>
                                <form action="{{ route($rp . '.social.disconnect', $key) }}" method="POST" class="flex-1" onsubmit="return confirm('Disconnect {{ $info['name'] }}?')">
                                    @csrf
                                    @method('DELETE')
                        ...
                        @else
                            <a href="{{ route($rp . '.social.connect', $key) }}" class="flex items-center justify-center gap-2 w-full px-4 py-3 text-sm font-medium text-white rounded-lg transition hover:opacity-90" style="background-color: {{ $info['color'] }}">
