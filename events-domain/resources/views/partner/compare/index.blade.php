            ...
            <div class="card p-6 mb-6">
                <form method="GET" class="flex flex-wrap gap-3 items-end">
                    <div class="input-group mb-0 min-w-[200px] flex-1">
                        <label class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">Search Services</label>
                        <input type="text" name="search" placeholder="Search services..." value="{{ request('search') }}" class="input-field">
                    </div>
                    <div class="input-group mb-0 min-w-[160px]">
                        <label class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">Price Type</label>
                        <select name="price_type" class="input-field">
                            <option value="">All Types</option>
                            <option value="fixed" {{ request('price_type') === 'fixed' ? 'selected' : '' }}>Fixed Price</option>
                    ...
                    @if(request()->anyFilled(['search', 'price_type']))
                        <a href="{{ route('partner.compare.index') }}" class="text-sm text-gray-500 hover:text-gray-700">Clear</a>
                    @endif
                </form>
            </div>
