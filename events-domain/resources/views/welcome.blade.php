        ...
        <div class="container-page">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
                @forelse($featuredEvents as $event)
                    <a href="{{ route('events.show', $event->slug ?? $event->id) }}" class="card-hover group">
                        <div class="relative h-48 overflow-hidden">
                            @if($event->cover_image_url)
                                <img src="{{ $event->cover_image_url }}" alt="{{ $event->title }}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" loading="lazy" onerror="this.parentElement.innerHTML='<div class=&quot;w-full h-full bg-gradient-to-br from-terracotta-400 to-terracotta-700&quot;></div>'">
                            @else
                        ...
                        <div class="p-5">
                            <div class="flex items-center gap-2 mb-3">
                                <span class="badge-primary text-xs">{{ $event->category->name ?? 'General' }}</span>
                                <span class="badge-success text-xs">Open</span>
                            </div>
                            <h3 class="font-bold text-lg text-gray-900 mb-2 group-hover:text-terracotta-600 transition line-clamp-1">{{ $event->title }}</h3>
                            <p class="text-gray-500 text-sm mb-4 line-clamp-2">{{ Str::limit($event->description, 120) }}</p>
                            ...
                            <div class="flex items-center justify-between pt-4 border-t border-gray-100">
                                <span class="text-terracotta-600 font-bold text-sm">{{ number_format($event->expected_audience ?? 0) }}+ Attendees</span>
