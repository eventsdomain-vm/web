<x-app-layout>
    <x-slot name="title">Edit {{ $page->title }} - Admin</x-slot>
    ...
    <div class="space-y-6 max-w-3xl">
        ...
        <form action="{{ route('admin.cms.update', $page) }}" method="POST" class="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
            @csrf
            @method('PUT')
            <div class="space-y-5">
                <div>
                ...
                <div x-data="{ tab: 'source' }">
                    ...
                    <div x-show="tab === 'source'" x-cloak>
                        <textarea x-ref="source" name="content" id="content" rows="16" required
                            class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm font-mono focus:ring-2 focus:ring-[#E35336] focus:border-transparent">{{ old('content', $page->content) }}</textarea>
                    </div>
                ...
                <div>
                    ...
                    <textarea name="meta_description" id="meta_description" rows="2"
                        class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-[#E35336] focus:border-transparent">{{ old('meta_description', $page->meta_description) }}</textarea>
                </div>
            ...
            <div class="flex justify-end gap-3 mt-6 pt-4 border-t border-gray-100">
                ...
                <button type="submit" class="px-4 py-2 bg-[#E35336] text-white rounded-lg hover:bg-[#9E3A26] text-sm font-medium">Update Page</button>
            </div>
        </form>
    </div>
</x-app-layout>
