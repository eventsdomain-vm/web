<aside
    ...
    class="fixed inset-y-0 left-0 z-40 flex flex-col bg-white border-r border-gray-200 transition-all duration-200 ease-in-out overflow-hidden" style="background-color: #ffffff !important;"
>
    {{-- Logo --}}
    <div class="h-16 shrink-0 flex items-center border-b border-gray-200 transition-all duration-200"
         :class="{ 'px-5 justify-start': isExpanded || isMobileOpen, 'px-0 justify-center': !(isExpanded || isMobileOpen) }"
    >
        <a href="{{ route('dashboard') }}" class="flex items-center gap-3">
            <img 
                x-show="isExpanded || isMobileOpen" 
                src="{{ asset('logo.png') }}" 
                alt="EventsDomain" 
        ...
        <nav class="flex-1 overflow-y-auto py-6 px-4 no-scrollbar">
            <div class="flex flex-col gap-0.5">
                @foreach($allGroups as $group)
                    ...
                    {{-- Group Children --}}
                    <div x-show="(isExpanded || isHovered || isMobileOpen) && (expandedGroups['{{ addslashes($group['label']) }}'] ?? true)" x-collapse.duration.200ms>
